...
絞り込みの方針は `pr-tidy-classifier` のサンプリングとは異なる。あちらは「全ハンクが安全か」の全数保証を必要とするが、こちらの判定は「**8 トリガーに触れる変更が 1 つでもあるか**」の探索である。全 diff を均等に読む必要はなく、ファイル一覧と PR 本文から当たりをつけ、`gh pr diff <PR番号> -- <path>` で個別に取得して読む。
