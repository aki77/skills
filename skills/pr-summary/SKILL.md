...
加えて、何を変えたか（What）だけでなく、**なぜその方法を選んだか（作者の設計判断・トレードオフ）**を、diffから読み取れる範囲で言語化する。複数の選択肢がありえた箇所では「Aではなく B を選んだ」という判断が伝わるように書く。判断の意図が出力に現れることで、レビュアーはその判断の妥当性を評価できる。diffから判断の根拠が読み取れない場合は推測で断定せず、その旨を残してレビュアーが確認できるようにする。
