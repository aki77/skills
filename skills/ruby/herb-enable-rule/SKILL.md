...
（`.herb.yml` 冒頭のコメント、および `fix: ... を修正し herb-lint ... を有効化` 形式のコミット履歴がその証跡）。
...
ユーザーがルール名を指定していなければ、`.herb.yml` の `enabled: false` 一覧から選ぶ（どれにするか確認する）。
...
履歴のスタイルに倣う。1ルールの有効化は1論理単位なので原則1コミット。
